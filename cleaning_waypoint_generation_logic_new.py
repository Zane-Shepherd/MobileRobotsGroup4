    ...
    def generate_waypoints(self, room_coordinates, w, entry_point):
        ...
        if clean_along_x == 0 and x == xmin:
            ...
            while x < xmax:
                ...
                x, y, step_vert, index = waypoint_step(vert=y, vert_min=ymin, vert_max=ymax, step_vert=step_vert, hor=x, step_hor=step_across, w=w, index=index)
            ...
            while y < ymax:
                waypoints.append([x,y])
                y, x, step_vert, index = waypoint_step(vert=x, vert_min=xmin, vert_max=xmax, step_vert=step_vert, hor=y, step_hor=step_across, w=w, index=index)
